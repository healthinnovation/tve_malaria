...
d <- dt_final %>%
  group_by(NOMBPROV) %>% 
  filter(!NOMBPROV %in% c("PUTUMAYO","UCAYALI","ALTO AMAZONAS","DATEM DEL MARAï¿½ON","MARISCAL RAMON CASTILLA")) %>% 
  summarise(
    slide_period_dfr(
      across(everything()),
      fecha,
...
d %>% 
  dplyr::group_by(NOMBPROV) %>% 
  dplyr::group_map(~my_plot(.x))
...
dt_final %>% 
  ...
  filter(!NOMBDIST %in% c("ALTO NANAY","ALTO TAPICHE","BALSAPUERTO","BARRANCA","BELEN","CAHUAPANAS","CAPELO", # PROBLEMA: todos los distritos tienen mucho cantidad de 0, para falciparum o para vivax
                          "CONTAMANA","EMILIO SAN MARTIN","FERNANDO LORES","INAHUAYA","INDIANA","IQUITOS",
                          "JEBEROS","JENARO HERRERA","LAGUNAS","LAS AMAZONAS","MANSERICHE","MAQUIA",
                          "MAZAN","MORONA","NAPO","NAUTA","PADRE MARQUEZ","PAMPA HERMOSA","PARINARI",
                          "PASTAZA","PEBAS","PUINAHUA","PUNCHANA","PUTUMAYO","RAMON CASTILLA",
                          "REQUENA","SAN JUAN BAUTISTA","SAN PABLO","SANTA CRUZ","SAQUENA","SARAYACU",
                          "SOPLIN","TAPICHE","TENIENTE CESAR LOPEZ ROJAS","TIGRE","TORRES CAUSANA",
                          "TROMPETEROS","URARINAS","VARGAS GUERRA","YAQUERANA","YAVARI","YURIMAGUAS")) %>% 
  summarise(
    slide_period_dfr(
      across(everything()),
      fecha,
